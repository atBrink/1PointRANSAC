function [optimalPose, optimalE, optimalConsensusSet] = twoPointRANSAC( ...
    x1, x2, K, errThreshold, iterations)
    
    m = size(x1,2);
    s = 2; % Number of points needed to reach a solution
    ...
    for i = 1:iterations
        ...
        [theta2, phi_c] = nonLinearTwoPointSolution(x1(:,randinds),x2(:,randinds));
        rho = 1;
        L = rho*((-sin((theta2/2)-phi_c))/(sin(phi_c)+sin(theta2-phi_c)));
